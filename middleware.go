...
var respCost = &Metric{
	ID:          "respCost",
	Name:        "response_cost_duration_seconds",
	Description: "The HTTP request-response cost time .",
	Type:        "histogram_vec",
	Args:        []string{"code", "method", "handler", "host", "url"}}

var respQuantileCost = &Metric{
	ID:          "respQuantileCost",
	Name:        "response_quantile_cost_duration_seconds",
	Description: "The HTTP request-response quantile cost time .",
	Type:        "summary_vec",
	Args:        []string{"code", "method", "handler", "host", "url"}}
...
type Prometheus struct {
	...
	respCost             *prometheus.HistogramVec
	respQuantileCost *prometheus.SummaryVec
	router               *gin.Engine
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		case respCost:
			p.respCost=metric.(*prometheus.HistogramVec)
		case respQuantileCost:
			p.respQuantileCost=metric.(*prometheus.SummaryVec)
		}
...
func (p *Prometheus) HandlerFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		p.respCost.WithLabelValues(status, c.Request.Method, c.HandlerName(), c.Request.Host, url).Observe(elapsed)
		p.respQuantileCost.WithLabelValues(status, c.Request.Method, c.HandlerName(), c.Request.Host, url).Observe(elapsed)
